...
你已经能让模型回答问题了。但当你想连续问两个有上下文关联的问题时（比如先问"我喜欢猫"，再问"它们为什么爱睡觉"），模型却"失忆"了——它不知道"它们"指什么。

原因藏在 `messages` 数组里。下一节我们就讲清楚：[messages 数组：对话是怎么组织的](./01-messages-intro.md)。

## 进阶篇会深入讲什么

本节只是跑通最简调用。想深入协议本身，去看进阶篇：

- [协议总览](../00-overview.md) —— API 设计哲学、行业标准地位、协议生命周期、速率限制响应头详解
- [响应格式](../02-response.md) —— 完整响应字段全解（含 `service_tier`、`refusal`、`logprobs` 等）
